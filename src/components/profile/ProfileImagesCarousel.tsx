...
export const ProfileImagesCarousel = ({
  ...
  return <div className="w-full max-w-md mx-auto mb-6">
      ...
      <div className="relative">
        {/* Left scroll button */}
        {canScrollLeft && <Button variant="outline" size="icon" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-todoDarkGray border-todoBlack text-white hover:bg-todoBlack" onClick={scrollLeft} aria-label="Прокрутить влево">
            <ChevronLeft size={16} />
          </Button>}
        
        {/* Scrollable thumbnails gallery */}
        <div className="overflow-hidden relative">
          <div ref={scrollContainerRef} className="flex space-x-3 overflow-x-auto scrollbar-hide py-2 px-1 scroll-smooth" style={{
          scrollbarWidth: 'none',
          msOverflowStyle: 'none'
        }}>
            {userImages.map((image, index) => <div key={image} className="flex flex-col items-center">
                ...
                <button onClick={() => handleDeleteImage(image)} className="p-1 text-red-600 hover:text-red-700 transition-colors" aria-label="Удалить изображение" title="Удалить изображение" type="button">
                  <Trash2 size={16} />
                </button>
              </div>)}
            
            {/* Empty state when no images */}
            {userImages.length === 0 && !showAddButton && <div className="flex-1 h-24 flex items-center justify-center text-todoMediumGray">
                Нет фотографий
              </div>}
          </div>
        </div>
        ...
        {/* Right scroll button */}
        {canScrollRight && <Button variant="outline" size="icon" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-todoDarkGray border-todoBlack text-white hover:bg-todoBlack" onClick={scrollRight} aria-label="Прокрутить вправо">
