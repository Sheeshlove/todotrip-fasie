  ...
  return <div className="flex justify-center -mt-10 relative z-10">
      <div className="relative flex items-center justify-center w-24 h-24">
        ...
        <svg className="absolute" width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          <circle className="text-gray-200/20" strokeWidth={strokeWidth} stroke="currentColor" fill="transparent" r={radius} cx={size / 2} cy={size / 2} />
        </svg>

        {/* Progress circle with animation */}
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="absolute transform -rotate-90 rounded-none bg-gray-950">
          <circle className="transition-all duration-1000 ease-in-out" strokeWidth={strokeWidth} strokeLinecap="round" stroke={strokeColor} fill="transparent" r={radius} cx={size / 2} cy={size / 2} style={{
        ...
        <div className="z-10 flex items-center justify-center">
          <span className={`text-center ${isGrayCircle || isLoading ? 'text-gray-300/90' : 'text-white'} font-bold ${isLoading ? 'animate-pulse' : ''}`} style={{
          fontSize: '1.1rem'
        }}>
